...
@shared_task
def cleanup_processing_jobs() -> Dict[str, Any]:
    """
    Clean up old AI processing jobs
    Delegates to ai_service cleanup task
    """
    try:
        from ai_service.tasks.ai_tasks import cleanup_expired_processing_jobs
        
        # Call ai_service cleanup
        result = cleanup_expired_processing_jobs.delay()
        
        return {
            'status': 'delegated_to_ai_service',
            'task_id': result.id,
            'triggered_at': timezone.now().isoformat()
        }
        
    except Exception as e:
        logger.error(f"Failed to trigger processing jobs cleanup: {str(e)}")
        return {'error': str(e)}
